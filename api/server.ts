...
app.get('/api/admin-install-shell', (_req, res) => {
  ...
  res.send(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>تثبيت SAT Control</title><style>body{margin:0;background:#f5f6f8;font-family:Arial,sans-serif;color:#172033}.wrap{min-height:100vh;display:grid;place-items:center;padding:20px}.card{width:min(440px,100%);background:#fff;border-radius:20px;padding:28px;box-shadow:0 18px 50px #0001}.brand{text-align:center;font-size:32px;font-weight:900;color:#b58a24}label{display:block;margin:14px 0 6px;font-weight:700}input{width:100%;box-sizing:border-box;padding:12px;border:1px solid #dce0e6;border-radius:10px}button{width:100%;margin-top:18px;padding:13px;border:0;border-radius:10px;background:#b58a24;color:#fff;font-weight:800}.muted{color:#6b7280}.err{color:#b91c1c}</style></head><body><div class="wrap"><form class="card" id="f"><div class="brand">SAT</div><h1>إنشاء حساب المدير</h1><p id="state" class="muted">جاري فحص قاعدة البيانات…</p><label>الاسم</label><input id="name" value="مدير النظام" required><label>البريد الإلكتروني</label><input id="email" type="email" placeholder="admin@example.com" required><label>كلمة المرور</label><input id="password" type="password" minlength="8" autocomplete="new-password" required><label>تأكيد كلمة المرور</label><input id="confirm" type="password" minlength="8" autocomplete="new-password" required><button id="submit">تثبيت وإنشاء الحساب</button><p id="error" class="err"></p></form></div><script>const f=document.getElementById('f'),stateEl=document.getElementById('state'),errorEl=document.getElementById('error'),submitEl=document.getElementById('submit'),nameEl=document.getElementById('name'),emailEl=document.getElementById('email'),passwordEl=document.getElementById('password'),confirmEl=document.getElementById('confirm');async function check(){try{const r=await fetch('/api/admin/install'),d=await r.json();if(!r.ok)throw new Error(d.message||'قاعدة البيانات غير جاهزة');if(d.data.installed){stateEl.textContent='تم التثبيت مسبقًا. سيتم نقلك لتسجيل الدخول.';submitEl.disabled=true;setTimeout(()=>top.location.hash='/admin-login',900)}else if(!d.data.sessionReady){stateEl.textContent='Neon متصل لكن SESSION_SECRET غير جاهز.';submitEl.disabled=true}else stateEl.textContent='Neon جاهز. اختر بيانات دخول لوحة التحكم.'}catch(e){stateEl.textContent='قاعدة البيانات غير جاهزة';errorEl.textContent=e.message}}f.addEventListener('submit',async e=>{e.preventDefault();errorEl.textContent='';const passwordValue=passwordEl.value.trim(),confirmValue=confirmEl.value.trim();if(passwordValue!==confirmValue){errorEl.textContent='كلمتا المرور غير متطابقتين';return}if(passwordValue.length<8){errorEl.textContent='كلمة المرور يجب أن تكون 8 أحرف على الأقل';return}submitEl.disabled=true;try{const r=await fetch('/api/admin/install',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:nameEl.value.trim(),email:emailEl.value.trim(),password:passwordValue})}),d=await r.json();if(!r.ok)throw new Error(d.message||'تعذر التثبيت');stateEl.textContent='تم إنشاء الحساب';setTimeout(()=>top.location.hash='/admin-login',700)}catch(e){errorEl.textContent=e.message;submitEl.disabled=false}});check();</script></body></html>`);
});
